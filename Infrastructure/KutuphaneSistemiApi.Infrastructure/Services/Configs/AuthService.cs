...
namespace KutuphaneSistemiApi.Infrastructure.Services.Configs
{
                ...
                var actions = controller.GetMethods().Where(m=>m.IsDefined(typeof(AuthorizeDefinitonAttribute)));
                    if(actions != null)
                    {
                        foreach (var action in actions)
                        {
                            ...
                            {
                                Menu menu = null;
                                var authorizeDefinitionAttribute = attributes.FirstOrDefault(a => a.GetType() == typeof(AuthorizeDefinitonAttribute))
                                    as AuthorizeDefinitonAttribute;
                                if (!menus.Any(m => m.Title == authorizeDefinitionAttribute.Menu))
                                {
                                    menu = new() { Title = authorizeDefinitionAttribute.Menu };
                                    menus.Add(menu);
                                }
                                else
                                    menu = menus.FirstOrDefault(m => m.Title == authorizeDefinitionAttribute.Menu);
                                Application.DTOs.Configuration.Action _action = new()
                                {
                                    ActionType = Enum.GetName(typeof(ActionType),authorizeDefinitionAttribute.ActionType),
                                    Definition = authorizeDefinitionAttribute.Definiton,
                                };
